 ...
 * グラス接続を監視し、接続時にGlassesMainActivityを自動起動する。
 */
...
class MainActivity : ComponentActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContent {
            GlimmerTheme {
                PhoneLauncherScreen(onLaunch = { launchGlassesActivity() })
            }
        }
...
@Composable
private fun PhoneLauncherScreen(onLaunch: () -> Unit) {
    Box(
        modifier = Modifier
            .fillMaxSize()
        ...
        Column(
            ...
            TitleChip { Text("Camera + Gemini QA") }
            Card(
                title = { Text("Visual Q&A") },
                action = {
                    Button(onClick = onLaunch) { Text("Launch") }
                },
            ) {
                Text("Connect AI glasses to start visual question answering")
            }
        }
    }
}
